/*
   ...
   This class draws the first background of the story that shows the old lady making the gingerbread man.
...
public class Background
{
    ...
    public void draw ()
    {
	//local colour variables for the background and old lady
	...
	{
	    c.setColor (peach);
	    c.drawOval (300 + x, 320 + x, 50 - 2 * x, 50 - 2 * x);
	}
	for (int x = 40 ; x > 0 ; x--)
	{
	...
	for (int x = 30 ; x > 0 ; x--)
	{
	    c.drawOval (285 + x, 330 + x, 30 - 2 * x, 30 - 2 * x);
	}
	//rolling pin
	for (int x = 0 ; x < 15 ; x++)
	{
	    c.setColor (woodBrown);
	    c.drawLine (260, 300 + x, 380, 300 + x);
	}
	for (int x = 0 ; x < 5 ; x++)
	{
	    c.drawLine (250, 305 + x, 390, 305 + x);
	}
	//hands
	for (int x = 0 ; x < 16 ; x++)
	{
	...
	//main captions and title
	c.setFont (new Font ("TimesRoman", Font.PLAIN, 24));
	c.drawString ("Once upon a time,", 410, 30);
	c.drawString ("a little old woman", 410, 50);
	c.drawString ("decided to make...", 410, 70);
	c.drawString ("a gingerbread man", 410, 90);
	c.setFont (new Font ("TimesRoman", Font.PLAIN, 12));
	c.setColor (darkBrown);
